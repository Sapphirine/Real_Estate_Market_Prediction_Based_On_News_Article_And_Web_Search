...
# based on sentiment score created from Google Search Query Data analysis
...
#make a dataFrame
data = {'date': date[0:116], 'Google_Search': hpi[0:116], 'senti': senti[0:116]}
df = pd.DataFrame(data, columns = ['date', 'Google_Search', 'senti'])
df['date'] = pd.to_datetime(df['date'])
df.index = df['date']
del df['date']

plt.figure()
plt.plot(df.index, df['Google_Search'])
plt.ylabel('Google_Search')
plt.title('2007-01 ~ 2016-08')

print(df)

model = pf.ARIMAX(data=df, formula='Google_Search~1+senti',ar=1, ma=1, family=pf.Normal())
x = model.fit("MLE")
x.summary()
model.plot_fit()
